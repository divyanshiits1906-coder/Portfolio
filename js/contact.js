...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const handleFormSubmit = (e) => {
    ...
    // Formspree endpoint configuration (Vite exposes VITE_ prefix env)
    const endpoint = import.meta.env.VITE_FORMSPREE_ENDPOINT || "https://formspree.io/f/mojownej";
    ...
    .then(response => {
      if (response.ok) {
        ...
        if (triggerText) {
          triggerText.textContent = "Select project type(s)";
    ...
    .catch(error => {
      if (errorMessage) {
        ...
        setTimeout(() => { errorMessage.classList.add("show"); }, 50);
      } else {
        alert(error.message);
      }
    })
  ...
  const initCustomSelect = () => {
    ...
    // Update trigger text based on selected options
    const updateTriggerText = () => {
      const selected = [];
      const hiddenInput = document.getElementById("projectTypeVal");
      checkboxes.forEach((cb) => {
        if (cb.checked) {
          const labelText = cb.closest(".custom-option").textContent.trim();
  ...
  initCustomSelect(); // Initialize custom multi-select
